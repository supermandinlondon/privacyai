...
function NewPrivacyRequirementChat() {
  ...
  const doc = await addDoc (
    collection(db,"users", session?.user?.email!, "chats"),{
      ...
      isPrivacyRequirement: true,
      createdAt: serverTimestamp()
    }
    );
    console.log("before routing");
    router.push(`/protected/client/privacyrequirementchat/${doc.id}`);
  ...
  return (
    <div onClick ={creteNewChat} className='border-gray-700 border chatRow' >
       <PlusIcon className='h-4 w-4'/>
        <p>New Privacy Requirement Chat</p>
    </div>
  );
}

export default NewPrivacyRequirementChat;